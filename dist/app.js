...
function render(options={}){
  ...
  document.getElementById('app').innerHTML=`<div class="statusbar" aria-hidden="true"><span>9:41</span><div class="island"></div><span class="status-icons">${icon('signal')}${icon('wifi')}${icon('battery')}</span></div>${!welcome?`<header class="app-header">${!root?A(icon('back')+'<span>Retour</span>','back',{label:'Revenir à l’écran précédent'},'flow-exit'):A(logo(),'home',{},'brand-button')}<span>${!root?logo():''}</span><div>${route.startsWith('ACH')?B(icon('bag')+(M.cart.length?`<b class="cart-count">${M.cart.reduce((n,c)=>n+c.qty,0)}</b>`:''),'ACH-02','icon-button',{label:'Panier, '+M.cart.reduce((n,c)=>n+c.qty,0)+' articles'}):B(icon('settings'),'PRF-04','icon-button',{label:'Paramètres'})}</div></header>`:''}<main data-route="${route}" class="screen mock-screen ${welcome?'welcome':'has-nav'}">${statusBanner()}${M.scenario==='unavailable'&&!/^ANA-0[3-8]$/.test(route)?empty('Cette inspiration n’est plus disponible','Vos autres contenus sont conservés.','DEC-01','Trouver une autre idée'):V[route]()}${selectionControl()}</main>${!welcome?nav():''}<div class="home-indicator"></div>`;
