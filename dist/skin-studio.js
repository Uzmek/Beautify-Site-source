...
function skinStudioShell(tab,body,result){
  return `<div class="lg-page beauty-page ss-shell sb-shell" data-moment="${esc(tab)}"><header class="ss-header">${A(icon('back'),'back',{label:'Retour'},'ss-icon-button')}<div><h1 tabindex="-1">Ma peau</h1></div>${A(icon('history'),'lg-history-domain',{domain:'Peau',label:'Historique de mes analyses de peau'},'ss-icon-button')}</header>`+
...
function skinStudioRoutine(moment,result){
  ...
  return `<div class="ss-routine sc-live-report${M.skinRoutineEditing?' is-editing':''}">${linkedBilan}${skinStudioSummary(moment,r)}`+
    ...
    `<div class="ss-routine-note">${!M.skinRoutineEditing&&counted.length&&!complete?A('Tout cocher','skin-check-all',{routine:r.id},'ss-quiet'):''}${B(icon('bell')+'<span>Rappel'+(M.reminders.routine?' : '+esc(M.reminders.routineTime||'08:00'):'')+'</span>','PRF-05','ss-quiet ux-routine-reminder',{label:'Régler le rappel de mes routines'})}<span>${all.length} soin${all.length>1?'s':''} dans votre routine</span></div></div>`;
}
